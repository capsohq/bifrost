...
func (g *GenericRouter) createHandler(config RouteConfig) fasthttp.RequestHandler {
	return func(ctx *fasthttp.RequestCtx) {
		...
		if err != nil {
			g.sendError(ctx, bifrostCtx, config.ErrorConverter, newBifrostErrorWithCode(err, "failed to convert request to Bifrost format", fasthttp.StatusInternalServerError))
			return
		}
		if bifrostReq == nil {
