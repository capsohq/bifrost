...
func TestIsEncryptedReasoningRejection(t *testing.T) {
	tests := []struct {
		...
		{
			// Anthropic thinking signatures are also stripped before the fail-soft
			// retry, so classify this rejection alongside encrypted content failures.
			name: "anthropic thinking signature rejection",
			err: &schemas.BifrostError{
				StatusCode: schemas.Ptr(400),
				Error: &schemas.ErrorField{
					Type:    schemas.Ptr("invalid_request_error"),
