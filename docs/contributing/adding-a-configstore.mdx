...
Any custom backend for config store should implement the `ConfigStore` interface. The interface is defined in [configstore/store.go](https://github.com/capsohq/bifrost/blob/main/framework/configstore/store.go).
...
GORM provides implementations for the functions listed in the `ConfigStore` interface. This significantly simplifies the implementation of the config store (see [postgres.go](https://github.com/capsohq/bifrost/blob/main/framework/configstore/postgres.go) as an example).
