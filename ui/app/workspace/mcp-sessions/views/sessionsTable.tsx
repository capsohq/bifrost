...
export default function SessionsTable({
  ...
  const [pendingDelete, setPendingDelete] = useState<MCPSessionRow | null>(
    null,
  );
  const [pendingActionRowId, setPendingActionRowId] = useState<string | null>(
    null,
  );
  ...
  const confirmRevoke = async () => {
    ...
    try {
      ...
      toast({
        title:
          row.kind === "header" ? "Header values revoked" : "Session revoked",
      });
    } catch (err) {
      toast({
        title:
          row.kind === "header"
            ? "Failed to revoke header values"
  ...
  return (
    <div className="space-y-4">
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            {pendingDelete?.kind === "header" ? (
              <>
                <AlertDialogTitle>
                  Revoke these stored header values?
                </AlertDialogTitle>
                <AlertDialogDescription>
                  Bifrost will remove the stored credential values for this
                  binding. There is no upstream token to revoke; the user will
              ...
              <>
                ...
                <AlertDialogDescription>
                  Bifrost will remove the stored credential for this binding.
                  The upstream OAuth token is not revoked at the provider — it
                  stays detached and expires naturally. Anyone using this
                  binding will need to re-authenticate to obtain a fresh token.
                </AlertDialogDescription>
              </>
            )}
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel data-testid="mcp-session-revoke-cancel">
              Cancel
            </AlertDialogCancel>
            <AlertDialogAction
              onClick={confirmRevoke}
              data-testid="mcp-session-revoke-confirm"
            >
              Revoke
            </AlertDialogAction>
          </AlertDialogFooter>
      ...
      <div className="flex items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground text-sm">
            Per-user credentials stored for MCP servers (OAuth tokens and
            submitted headers), plus any pending authentication flows.
          </p>
        </div>
      </div>
      ...
      <div
        className={`overflow-auto rounded-sm border ${isFetching ? "opacity-70 transition-opacity" : ""}`}
      >
        <Table>
          <TableHeader>
            <TableRow>
                ...
                <TableCell colSpan={7} className="h-24 text-center">
                  {hasActiveFilters ? (
                    ...
                    <span className="text-muted-foreground text-sm">
                      No sessions yet. Sessions appear here when an inference
                      request or MCP gateway call triggers per-user
                      authentication (OAuth or header submission).
                    </span>
                  )}
                </TableCell>
              </TableRow>
              ...
              sessions.map((row) => (
                <TableRow key={`${row.kind}-${row.id}`} className="group">
                  <TableCell className="font-medium">
                    {row.mcp_client?.name || row.mcp_client?.client_id || "-"}
                  </TableCell>
                  <TableCell>
                    ...
                    <div className="flex flex-col">
                      ...
                      {row.last_refreshed_at && (
                        <span className="text-xs">
                          refreshed {formatRelativePast(row.last_refreshed_at)}
                        </span>
                      )}
                    </div>
                  </TableCell>
...
function StatusBadge({ status }: { status: string }) {
  ...
  if (status === "needs_update") {
    ...
    return (
      <Badge
        variant="outline"
        className="border-red-500 bg-red-100 text-red-900 dark:bg-red-900/30 dark:text-red-200"
      >
        Needs update
      </Badge>
    );
