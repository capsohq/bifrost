...
import (
	...
	configstore "github.com/capsohq/bifrost/framework/configstore"
...
func TestPricingE2E_Step3_FileConfigValid(t *testing.T) {
	...
	// Resolution log must include the concrete file values.
	assert.True(t, log.hasLog("resolved pricing config", "url=https://", "sync_interval=7200 seconds"),
		"expected resolved pricing config log with concrete file values\n%s", log.dump())
