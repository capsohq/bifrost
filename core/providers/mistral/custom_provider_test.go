...
func TestParseMistralError_UsesExportedConverterMetadata(t *testing.T) {
	...
	bifrostErr := ParseMistralErrorWithMetadata(resp, customMistralProviderName, schemas.ChatCompletionRequest, "mistral-small-latest")
	...
	assert.Equal(t, "mistral-small-latest", bifrostErr.ExtraFields.OriginalModelRequested)
}
